...
shinyServer(function(input, output) { 
  ...
  output$sortby <- renderUI({
    selectInput("sortby", label = "Sort By", 
                choices = c("Price", "Points"))
  })
  
  output$sortfulltableby <- renderUI({
    selectInput("sortfulltableby", label = "Sort By", 
                choices = c("Price", "Points"))
  })
  ...
  wine_fulltable <- reactive({
    if(input$sortfulltableby == "Points"){
      wine_data() %>% 
        select(Name, Price, Points, Description, Country, Variety) %>% 
        arrange(desc(Points))
    } else{
